...
namespace CryptoNote {
	...
	bool Currency::getBlockReward(uint8_t blockMajorVersion, size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins,
		...
		// instamine to pay parsicoin holders from the old chain after swap & 8% for the devs
		if (alreadyGeneratedCoins == 0) {
			baseReward = m_moneySupply*0.0000001; //1.5 PARS on block 0
		}
		if (alreadyGeneratedCoins == m_moneySupply*0.0000001) {
			baseReward = m_moneySupply*0.48; //48% on block 1
		}
